...
export const getProducts = async (req, res) => {
  try {
    ...
    // ✅ brand filter (comma separated array)
    if (brand && String(brand).trim()) {
      const bArr = String(brand).split(',').map(b => new RegExp(`^${escapeRegex(b.trim())}$`, "i"));
      filter.brand = { $in: bArr };
    }
    ...
    if (rating && String(rating).trim()) {
      const rArr = String(rating).split(',').map(Number).filter(n => !isNaN(n));
      if (rArr.length > 0) filter.rating = { $in: rArr };
    }
